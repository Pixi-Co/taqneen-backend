$(document).ready(function() {
    ...
    $('#customer_id').on('select2:select', function(e) {
        ...
        // view customer data
        $('.customer_name').html(data.text);
        $('.customer_address').html(data.address_line_1);
        $('.customer_email').html(data.email);
        $('.customer_mobile').html(data.mobile);
        $('.customer_city').html(data.city);
        $('.customer_country').html(data.country);
        $('.customer_zipcode').html(data.zip_code);
    });
    ...
    $('#add-payment-row').click(function() {
        ...
        $.ajax({
            method: 'POST',
            url: '/sub/sells/pos/get_payment_row',
    ...
    $(document).on('change', "#class_type_id, #transaction_type", function() {
        //Show product list.
...
function changePos(type) {
    if (type == 'subscription') {
        $("#product_brand_div, #restaurant_module_span, #show_featured_products").hide();
        $('#class_type_div, #session_div').show();
        $('#class_type_div, #session_div').val('');
    } else {
        $("#product_brand_div, #restaurant_module_span, #show_featured_products").show();
        $('#class_type_div, #session_div').hide();
        $('#class_type_div, #session_div').val('');
    }
}
...
function get_featured_products() {
    ...
    if (location_id && $('#featured_products_box').length > 0) {
        $.ajax({
            method: 'GET',
            url: '/sub/sells/pos/get-featured-products/' + location_id,
            dataType: 'html',
            success: function(result) {
                if (result) {
...
function get_product_suggestion_list(category_id, brand_id, location_id, url = null, is_enabled_stock = null, repair_model_id = null) {
    ...
    if (url == null) {
        url = '/sub/sells/pos/get-product-suggestion';
    }
...
function get_recent_transactions(status, element_obj) {
    ...
    $.ajax({
        method: 'GET',
        url: '/sub/sells/pos/get-recent-transactions',
...
function pos_product_row(variation_id = null, purchase_line_id = null, weighing_scale_barcode = null, quantity = 1) {
    ...
    if (add_via_ajax) {
        ...
        $.ajax({
            method: 'GET',
            url: '/sub/sells/pos/get_product_row/' + variation_id + '/' + location_id,
            async: false,
            data: {
...
function pos_print(receipt) {
    ...
    if (receipt.print_type == 'printer') {
        ...
        // new code
        new_print_receipt(receipt.html_content);
    }
}
...
function getCustomerRewardPoints() {
    ...
    $.ajax({
        method: 'POST',
        url: '/sub/sells/pos/get-reward-details',
        data: {
            customer_id: customer_id
        },
        dataType: 'json',
...
$(document).on('change', '#types_of_service_id', function() {
    ...
    if (types_of_service_id) {
        $.ajax({
            method: 'POST',
            url: '/sub/sells/pos/get-types-of-service-details',
            data: {
