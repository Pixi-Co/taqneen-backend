...
function reloadProductForClassType() {
    //Show product list.
    get_product_suggestion_list(
        $('select#product_category').val(),
        $('select#product_brand').val(),
...
function changePos(type) {
    if (type == 'subscription') {
        $("#product_category_div, #product_brand_div, #restaurant_module_span, #show_featured_products").hide();
        $('#class_type_div, #session_div').show();
        $('#class_type_div, #session_div').val('');
    } else {
        $("#product_category_div, #product_brand_div, #restaurant_module_span, #show_featured_products").show();
...
function pos_print(receipt) {
    ...
    if (receipt.print_type == 'printer') {
        ...
        // new code
        var width = Math.round((window.innerWidth / 3) * 2);
        var height = window.innerHeight - 50;
        var left = Math.round(window.innerWidth / 5);
        var printWindowOptions = "toolbar=yes,scrollbars=yes,resizable=yes,top=100,left=" + left + ",width=" + width + ",height=" + height + "";
        var printWindow = window.open("", "print invoice", printWindowOptions);
        printWindow.document.write(receipt.html_content);
        __currency_convert_recursively(printWindow.document.body);
    }
}
