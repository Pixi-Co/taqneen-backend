 ...
 * ==========================================================
 *
 * Tickets App main file. © 2017-2022 board.support. All rights reserved.
 *
...
<div class="sb-main sb-tickets sb-loading sb-load<?php echo $css ?>" data-height="<?php echo sb_get_setting('tickets-height') ?>" data-offset="<?php echo sb_get_setting('tickets-height-offset') ?>">
    <div class="sb-tickets-area" style="visibility: hidden; opacity: 0;">
        ...
        <div class="sb-panel-left">
            <div class="sb-top">
                <div>
                    <?php if (!sb_isset($disable_fields, 'tickets-button')) echo '<div class="sb-btn sb-icon sb-new-ticket"><i class="sb-icon-plus"></i>' . sb_($button_name ? $button_name : 'Create Ticket') .'</div>'; else echo '<div class="sb-title">' . sb_('Tickets') . '</div>'; ?>
                </div>
...
function sb_tickets_email($message = false, $attachments = false) {
    if (empty($message) && empty($attachments)) return false;
    $user_email = sb_isset(sb_get_active_user(), 'email');
    $email = sb_get_multilingual_setting('emails', 'tickets-email', sb_get_user_language());
    if ($user_email && !empty($email['tickets-email-subject'])) {
        $t= sb_email_send($user_email, sb_merge_fields($email['tickets-email-subject']),  sb_merge_fields(str_replace(['{message}', '{attachments}'], [$message, sb_email_attachments_code($attachments)], $email['tickets-email-content'])));
        return $t;
    }
    return false;
}
