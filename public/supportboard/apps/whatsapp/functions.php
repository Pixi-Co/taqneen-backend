...
function sb_whatsapp_rich_messages($message, $extra = false) {
    ...
    if ($shortcode) {
        ...
        switch ($shortcode_name) {
            ...
            case 'buttons':
            case 'chips':
                $human_takeover = $shortcode_id == 'sb-human-takeover';
                ...
                if ($twilio) {
                    ...
                    $is_buttons = $count < 4;
                    $message = ['type' => $is_buttons ? 'button' : 'list', 'body' => ['text' => $shortcode['message']]];
                ...
                if ($human_takeover && defined('SB_DIALOGFLOW')) {
...
function sb_whatsapp_360_synchronization($key = false, $cloud = '') {
    return sb_whatsapp_360_curl('configs/webhook', ['url' => SB_URL . '/apps/whatsapp/post.php' . str_replace(['&', '='], ['%3F', '%3D'], $cloud)]);
}
