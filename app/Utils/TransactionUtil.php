...
class TransactionUtil extends Util
{
    ...
    public function createSellTransaction($business_id, $input, $invoice_total, $user_id, $uf_data = true)
    {
        ...
        $transaction = Transaction::create([
            ...
            'is_subscription' => isset($input['is_subscription'])? $input['is_subscription'] : 0, 
        ...
        // create installment if exists
        $this->createSellInstallment($business_id, $transaction->id, $input['contact_id'], $user_id);

        return $transaction;
    }

    public function createSellInstallment($business_id, $transaction_id, $contact_id, $user_id) {
        $index = 0;
        try {
            foreach(request()->installment_dates as $date) {
                /**
                 * 	business_id	transaction_id	contact_id	value	date	notes
                 */
                DB::table('installments')->insert([
                    ...
                    "user_id" => $user_id,
                    "date" => $date,
                    "value" => request()->installment_values[$index],
        ...
        foreach ($products as $product) {
            ...
            if (!empty($product['transaction_sell_lines_id'])) {
                ...
                $line = [
                    ...
                    'class_type_id' => $product['class_type_id']?? null,
                    'session_id' => $product['session_id']?? null,
                ];

                
                // add member to session
                if (isset($product['class_type_id']) && isset($product['session_id']))
                    Subscription::addMemeberToSession(request()->contact_id, $product['session_id'], $product['class_type_id']);
        ...
        if (!empty($payments_formatted)) {
            ...
            foreach ($transaction->payment_lines as $key => $value) {
                if (!empty($account_transactions[$key])) {
                    ...
                    if (isset($account_transactions[$key]['card_type']))
                        $account_transactions[$key]['type'] = $account_transactions[$key]['card_type'];
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        $business_id = Auth::user()? Auth::user()->business_id : $transaction->business_id;
        $url = $this->getInvoiceUrl($transaction_id, $business_id);
        ...
        $output = [
            ...
            'commercial_number' => $il->commercial_number,
            'tax_number' => $il->tax_number,
            'url' => $url,
        ];
        ...
        $output['qrcode_img'] = $il->qrcode_img != 0 && !empty($il->qrcode_img) && file_exists(public_path('uploads/invoice_logos/' . $il->qrcode_img)) ? asset('uploads/invoice_logos/' . $il->qrcode_img) : false;
        ...
        if ($il->show_alternate_number == 1 && !empty($location_details->alternate_number)) {
            if (empty($output['contact'])) {
                $output['contact'] .= "<br>" . __('contact.mobile') . ': ' . $location_details->alternate_number;
            } else {
                $output['contact'] .= '<br> ' . $location_details->alternate_number;
            }
        }
        ...
        if ($il->show_customer == 1) {
            ...
            if (!empty($output['customer_name']) && $receipt_printer_type != 'printer') {
                ...
                if (!empty($customer->landline)) {
                    $output['customer_info'] .= '<br> ' . $customer->landline;
                }
            }
    ...
    */
    ...
    public function getLedgerDetails($contact_id, $start, $end)
    {
        ...
        $transactions = $this->__transactionQuery($contact_id, $start, $end)
                            ->select(
                                ...
                                DB::raw('(select id from contacts where contacts.id = contact_id) as contact_id'),
                            )
                            ->with(['location'])->get();
        ...
        $payments = $this->__paymentQuery($contact_id, $start, $end)
                        ->select(
                            ...
                            't.ref_no', 
                            't.invoice_no',
                            DB::raw('(select name from contacts where contacts.id = t.contact_id) as contact_name'),
                            DB::raw('(select id from contacts where contacts.id = t.contact_id) as contact_id'),
                            )
                        ->get();
    ...
    public function createExpense($request, $business_id, $user_id, $format_data = true, $data=null, $expense_payments=null)
    {
    ...
    public function updateAccountBalance($transaction, $business_id=null) {
        $business_id = $business_id? $business_id : session("business.id");
        $discountAccount = Account::where('is_for_discount', '1')->where('business_id', $business_id)->first();
        $taxAccount = Account::where('is_for_tax', '1')->where('business_id', $business_id)->first();
        $payments = [];
        $lines = [];
        $cardType = "";
        $discounts = 0;
        ...
        if ($transaction->type == 'purchase') {
            $lines = $transaction->purchase_lines()->get();
            $cardType = "debit";
        }
        ...
        foreach($lines as $line) {
            ...
            if (in_array($transaction->type, ['sell', 'sell_return'])) {
                ...
                $totalWithTax = $line->quantity * $line->unit_price_inc_tax;
            ...
            if (in_array($transaction->type, ['purchase', 'purchase_return'])) {
                $total = $line->quantity * $line->purchase_price;
                $totalWithTax = $line->quantity * $line->purchase_price_inc_tax; 

                $tax = ($totalWithTax - $total);
                $discount = $line->pp_without_discount * $line->pp_without_discount;
                $finalTotal = ($total - $discount) + $tax; 
            }


            $account_id = optional($line->product)->account_id > 0? optional($line->product)->account_id : optional(optional($line->product)->category)->account_id;
        
            if ($account_id > 0 && $finalTotal > 0) {
                $payments[] = [
                    "amount" => $finalTotal,
                    "method" => "cash",
        ...
        // discount
        if ($transaction->discount_amount > 0 && $discountAccount) {
            $payments[] = [
                ...
                "card_holder_name" => null,
                "card_transaction_number" => null,
                "card_type" => $cardType == 'credit'? 'debit' : 'credit',
                "card_month" => null,
                "card_year" => null,
                "card_security" => null,
                "cheque_number" => null,
        ...
        // tax
        if ($transaction->tax_amount > 0 && $taxAccount) {
            $payments[] = [
        ...
        foreach($payments as $pay) {
            request()->credit_debit = $pay['card_type'];
            $this->createOrUpdatePaymentLines($transaction, [$pay]);
        }
