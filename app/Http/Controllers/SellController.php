...
class SellController extends Controller
{
    ...
    public function index()
    {
        ...
        if (request()->ajax()) {
            ...
            // select query
            $sells->selectRaw('(select cmmsn_percent from users where transactions.commission_agent = users.id limit 1) commission_percent');
            $sells->selectRaw('(((select cmmsn_percent from users where transactions.commission_agent = users.id limit 1) / 100) * transactions.final_total) as commission_percent_calculated');
            ...
            $datatable = Datatables::of($sells->latest('transactions.created_at'))
                ...
                ->editColumn(
                    'commission_percent_calculated', function ($row)
                    {
                        return '<span class="commission_percent_calculated" data-orig-value="'.$row->commission_percent_calculated.'">'.format_currency($row->commission_percent_calculated).'</span>';
                    }
                )
                ->editColumn(
                    'commission_percent', function ($row)
                    {
                        return $row->commission_percent? intval($row->commission_percent) . " %" : '';
                    }
                )
                ->editColumn(
                    'tax_amount',
            ...
            $rawColumns = ['commission_percent_calculated', 'final_total', 'action', 'mass_delete', 'total_paid', 'total_remaining', 'payment_status', 'invoice_no', 'discount_amount', 'tax_amount', 'total_before_tax', 'shipping_status', 'types_of_service_name', 'payment_methods', 'return_due', 'conatct_name', 'settle'];
                ...
                ->addColumn(
                    ...
                    '<div class="btn-group">
                        ...
                        <ul class="dropdown-menu dropdown-menu-right" role="menu">
                            <li>
                            <a href="#" data-href="{{action(\'SellController@show\', [$id])}}" class="btn-modal" data-container=".view_modal"><i class="fas fa-eye" aria-hidden="true"></i> @trans("messages.view")</a>
                            </li>
                    @if($is_direct_sale == 1)
                        <li>
                        <a target="_blank" href="{{action(\'SellController@edit\', [$id])}}"><i class="fas fa-edit"></i>  @trans("messages.edit")</a>
                        </li>
                    @else
                    <li>
                    <a target="_blank" href="{{action(\'SellPosController@edit\', [$id])}}"><i class="fas fa-edit"></i>  @trans("messages.edit")</a>
                    </li>
                    @endif
                    <li>
                        <a href="#" class="print-invoice" data-href="{{route(\'sell.printInvoice\', [$id])}}"><i class="fas fa-print" aria-hidden="true"></i> @trans("messages.print")</a>
                    </li> 
                    @if( (auth()->user()->can("sell.create") || auth()->user()->can("direct_sell.access")) && config("constants.enable_convert_draft_to_invoice"))
                        <li>
                        <a href="{{action(\'SellPosController@convertToInvoice\', [$id])}}" class="convert-draft"><i class="fas fa-sync-alt"></i>  @trans("lang_v1.convert_to_invoice")</a>
                        </li> 
                    @endif
                    @if($sub_status != "proforma")
                    <li>
                    <a href="{{action(\'SellPosController@convertToProforma\', [$id])}}" class="convert-to-proforma"><i class="fas fa-sync-alt"></i>  @trans("lang_v1.convert_to_proforma")</a>
                    </li> 
                    @endif
                    <li>
                    <a href="{{action(\'SellPosController@destroy\', [$id])}}" class="delete-sale"><i class="fas fa-trash"></i>  @trans("messages.delete")</a>
                    </li>
                    @if($sub_status == "quotation")
                    <li><a href="{{action("SellPosController@showInvoiceUrl", [$id])}}" class="view_invoice_url"><i class="fas fa-eye"></i> @trans("lang_v1.view_quote_url")</a></li>
