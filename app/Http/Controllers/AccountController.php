...
class AccountController extends Controller
{
    ...
    public function index()
    {
        ...
        if (request()->ajax()) {
            ...
            return DataTables::of($accounts)
                ...
                ->editColumn('is_for_discount', function ($row) {
                    return $row->is_for_discount == '1'? 
                        '<span class="fa fa-check w3-text-green" ></span>' : 
                        '<span class="fa fa-close w3-text-red" ></span>'; 
                })
                ->editColumn('is_for_tax', function ($row) {
                    return $row->is_for_tax == '1'? 
                        '<span class="fa fa-check w3-text-green" ></span>' : 
                        '<span class="fa fa-close w3-text-red" ></span>'; 
                })
                ...
                ->rawColumns(['action', 'balance', 'name', 'is_for_discount', 'is_for_tax'])
                ->make(true);
        }
    ...
    public function store(Request $request)
    {
        ...
        if (request()->ajax()) {
            try {
                $input = $request->only(['is_for_discount', 'is_for_tax', 'name', 'account_number', 'note', 'account_type_id']);
                $business_id = $request->session()->get('user.business_id');
                $user_id = $request->session()->get('user.id');
                $input['business_id'] = $business_id;
                $input['created_by'] = $user_id;
                $chart = AccountType::where('business_id', $business_id)->where('code', $input['account_type_id'])->first();

                $input['account_type_id'] = optional($chart)->id;
    ...
    public function edit($id)
    {
        ...
        if (request()->ajax()) {
            ...
            $account = Account::where('business_id', $business_id)
                ->find($id);
            $chart = AccountType::where('business_id', $business_id)->where('id', $account->account_type_id)->first();
            $account->account_type_code = optional($chart)->code;
    ...
    public function update(Request $request, $id)
    {
        ...
        if (request()->ajax()) {
            try {
                ...
                $chart = AccountType::where('business_id', $business_id)->where('code', $input['account_type_id'])->first();
                $account->account_type_id = optional($chart)->id;
                $account->name = $input['name'];
                $account->is_for_discount = $input['is_for_discount']?? '0';
                $account->is_for_tax = $input['is_for_tax']?? '0';
