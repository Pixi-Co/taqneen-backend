...
class MemberShipController extends Controller
{
    ...
    {
        if (!auth()->user()->can('subscription.memberships.view') && !auth()->user()->can('subscription.memberships.create')) {
        ...
        if (request()->ajax()) {
            ...
            $products = $query->select(
                ...
                'products.class_type_id',  
                'products.subscription_number',  
            ...
            return Datatables::of($products)
                ...
                ->addColumn(
                    ...
                    function ($row) use ($selling_price_group_count) {
                        ...
                        if (auth()->user()->can('subscription.memberships.view')) {
                            $html .=
                            '<li><a href="' . action('ProductController@view', [$row->id]) . '" class="view-product"><i class="fa fa-eye"></i> ' . __("messages.view") . '</a></li>';
                        }

                        if (auth()->user()->can('subscription.memberships.update')) {
                            $html .=
                            '<li><a href="javascript:editMembership('.$row->id.')" _href="' . action('MemberShipController@edit', [$row->id]) . '"><i class="glyphicon glyphicon-edit"></i> ' . __("messages.edit") . '</a></li>';
                        }

                        if (auth()->user()->can('subscription.memberships.delete')) {
                            $html .=
                            '<li><a href="' . action('MemberShipController@destroy', [$row->id]) . '" class="delete-product"><i class="fa fa-trash"></i> ' . __("messages.delete") . '</a></li>';
                        }
                        ...
                        if ($row->enable_stock == 1 && auth()->user()->can('subscription.memberships.opening_stock')) {
                            $html .=
                        ...
                        if (auth()->user()->can('subscription.memberships.create')) {
                            ...
                            $html .=
                                '<li><a href="' . action('MemberShipController@create', ["d" => $row->id]) . '"><i class="fa fa-copy"></i> ' . __("lang_v1.duplicate_product") . '</a></li>';
                        }
                ...
                ->editColumn('type', '@trans("lang_v1." . $type)')
                ...
                ->setRowAttr([
                    'data-href' => function ($row) {
                        if (auth()->user()->can("membership.view")) {
                            return  action('ProductController@view', [$row->id]) ;
                        } else {
                            return '';
                        }
                    }])
                ->rawColumns(['action', 'image', 'mass_delete', 'product', 'selling_price'])
                ->make(true);
        }
    ...
    public function store(Request $request)
    {
        ...
        try {
            $business_id = $request->session()->get('user.business_id');
            $form_fields = ['stop_max_times', 'stop_max_days', 'days', 'subscription_number', 'class_type_id', 'name', 'brand_id', 'unit_id', 'category_id', 'tax', 'type', 'barcode_type', 'sku', 'alert_quantity', 'tax_type', 'weight', 'product_custom_field1', 'product_custom_field2', 'product_custom_field3', 'product_custom_field4', 'product_description', 'sub_unit_ids'];
            ...
            if (DB::table('products')->where('business_id', $business_id)->where('name', $request->name)->exists())
                return responseJson(0, __('product already exists'));
            ...
            DB::commit();
            $output = ['success' => 1,
                            'msg' => __('membership.product_added_success')
                        ];
        ...
        return [
            'status' => 1,
            "message" => __('membership.product_added_success'),
            "data" => $product
        ];
        //return redirect('products')->with('status', $output);
    }
    ...
    public function show($id)
    {
        ...
        return view('membership.show')->with(compact('details'));
    }
    ...
    public function edit($id)
    {
        ...
        return view('membership.form_edit')
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            $business_id = $request->session()->get('user.business_id');
            $product_details = $request->only(['stop_max_times', 'stop_max_days', 'days', 'subscription_number', 'name', 'brand_id', 'unit_id', 'category_id', 'tax', 'barcode_type', 'sku', 'alert_quantity', 'tax_type', 'weight', 'product_custom_field1', 'product_custom_field2', 'product_custom_field3', 'product_custom_field4', 'product_description', 'sub_unit_ids']);
            ...
            $product->subscription_number = isset($product_details['subscription_number'])? $product_details['subscription_number'] : null;
            $product->days = isset($product_details['days'])? $product_details['days'] : null;
            $product->stop_max_times = isset($product_details['stop_max_times'])? $product_details['stop_max_times'] : null;
            $product->stop_max_days = isset($product_details['stop_max_days'])? $product_details['stop_max_days'] : null;
            $product->brand_id = isset($product_details['brand_id'])? $product_details['brand_id'] : null;
            $product->unit_id = $product_details['unit_id'];
            $product->category_id = isset($product_details['category_id'])? $product_details['category_id'] : null;
            $product->tax = isset($product_details['tax'])? $product_details['tax'] : null;
            $product->barcode_type = isset($product_details['barcode_type'])? $product_details['barcode_type'] : null;
            $product->sku = isset($product_details['sku'])? $product_details['sku'] : null;
            $product->alert_quantity = isset($product_details['alert_quantity'])? $product_details['alert_quantity'] : null;
            $product->tax_type = isset($product_details['tax_type'])? $product_details['tax_type'] : null;
            $product->weight = isset($product_details['weight'])? $product_details['weight']: null;
            $product->product_custom_field1 = isset($product_details['product_custom_field1'])? $product_details['product_custom_field1'] : null;
            $product->product_custom_field2 = isset($product_details['product_custom_field2'])? $product_details['product_custom_field2'] : null;
            $product->product_custom_field3 = isset($product_details['product_custom_field3'])? $product_details['product_custom_field3'] : null;
            $product->product_custom_field4 = isset($product_details['product_custom_field4'])? $product_details['product_custom_field4'] : null;
            ...
            DB::commit();
            $output = ['success' => 1,
                            'msg' => __('membership.product_updated_success')
                        ];
        } catch (Exception $e) {
            DB::rollBack();
            \Log::emergency("File:" . $e->getFile(). "Line:" . $e->getLine(). "Message:" . $e->getMessage());
        ...
        */
        ...
        return responseJson(1, __('membership.product_updated_success'), $product);
        //return redirect('products')->with('status', $output);
    }
    ...
    public function getProductVariationFormPart(Request $request)
    {
        ...
        if ($request->input('action') == "add") {
            if ($request->input('type') == 'single') {
                return view('membership.partials.single_product_form_part')
                ...
                return view('membership.partials.variable_product_form_part')
                        ->with(compact('variation_templates', 'profit_percent', 'action'));
            } elseif ($request->input('type') == 'combo') {
                return view('membership.partials.combo_product_form_part')
            ...
            if ($request->input('type') == 'single') {
                ...
                return view('membership.partials.edit_single_product_form_part')
    ...
    public function getVariationValueRow(Request $request)
    {
        ...
        return view('membership.partials.variation_value_row')
    ...
    public function getProductVariationRow(Request $request)
    {
        ...
        return view('membership.partials.product_variation_row')
    ...
    public function getVariationTemplate(Request $request)
    {
        ...
        return view('membership.partials.product_variation_template')
    ...
    public function getComboProductEntryRow(Request $request)
    {
        if (request()->ajax()) {
            ...
            if (!empty($product_id)) {
                ...
                return view('membership.partials.combo_product_entry_row')
    ...
    public function quickAdd()
    {
        ...
        return view('membership.partials.quick_add_product')
    ...
    public function view($id)
    {
        ...
        try {
            ...
            return view('membership.view-modal')->with(compact(
                'product',
                'rack_details',
                'allowed_group_prices',
                'group_price_details',
    ...
    /**
     * Shows form to add selling price group prices for a membership.
     *
     * @param  int  $id
     * @return Response
     */
    public function addSellingPrices($id)
    {
        ...
        return view('membership.add-selling-prices')->with(compact('product', 'price_groups', 'variation_prices'));
    }

    /**
     * Saves selling price group prices for a membership.
     *
     * @param  \Illuminate\Http\Request  $request
     * @return Response
     */
    ...
    public function viewGroupPrice($id)
    {
        ...
        return view('membership.view-product-group-prices')->with(compact('product', 'allowed_group_prices', 'group_price_details'));
    }
    ...
    public function bulkEdit(Request $request)
    {
        ...
        if (!empty($selected_products_string)) {
            ...
            return view('membership.bulk-edit')->with(compact(
                'products',
                'categories',
                'brands',
                'taxes',
    ...
    public function getProductToEdit($product_id)
    {
        ...
        return view('membership.partials.bulk_edit_product_row')->with(compact(
            'product',
            'categories',
            'brands',
            'taxes',
    ...
    public function productStockHistory($id)
    {
        ...
        if (request()->ajax()) {
            ...
            return view('membership.stock_history_details')
        ...
        return view('membership.stock_history')
