        ...
        if (request()->ajax()) {
            ...
            return DataTables::of($query)
                    ...
                    ->addColumn('debit_credit', function ($row) {
                        $debit_credit = "";
                        if (in_array($row->type, ['expense', 'production_purchase', 'purchase', 'sell_return'])) 
                            $debit_credit = __('credit');
                        else    
                            $debit_credit = __('debit');
                    ...
                    ->addColumn('transaction_number', function ($row) {
                        $html = $row->ref_no;
                        if ($row->type == 'sell') {
                            $html = '<button type="button" class="btn btn-link btn-modal w3-text-black"
