...
class ReportController extends Controller
{
    ...
    {
        if (!auth()->user()->can('subscription.report.module')) {
            abort(403, 'Unauthorized action.');
        } 

        return view('report.pages.subscription_report');
    }
    ...
    public function ledger(Request $request) {
        ...
        return view("report.legder", compact('type'));
    }
    ...
    public function getTrialBalance(Request $request)
    {
        ...
        $location_id = $request->get('location_id') ? $request->get('location_id') : null;
        //
        $customerIds = DB::table('contacts')->where('type', 'customer')->where('business_id', $business_id)->pluck('id')->toArray();
        $supplierIds = DB::table('contacts')->where('type', 'supplier')->where('business_id', $business_id)->pluck('id')->toArray();
        $data = [
            "customer" => [
                "start" => DB::table('transactions')->where('business_id', $business_id)->where('type', 'opening_balance')->whereIn('contact_id', $customerIds)->sum('final_total'),
                "process" => DB::table('transactions')->where('business_id', $business_id)->where('type', '!=', 'opening_balance')->whereIn('contact_id', $customerIds)->sum('final_total'),
            ], 
            "supplier" => [
                "start" => -1 * DB::table('transactions')->where('business_id', $business_id)->where('type', 'opening_balance')->whereIn('contact_id', $supplierIds)->sum('final_total'),
                "process" => DB::table('transactions')->where('business_id', $business_id)->where('type', '!=', 'opening_balance')->whereIn('contact_id', $supplierIds)->sum('final_total'),
            ],
        ];

        $expenses = DB::table('expense_categories')->where('business_id', $business_id)->get();

        foreach($expenses as $item) {
            $data[$item->name] = [
                "start" => 0, 
                "process" => -1 * DB::table('transactions')->where('business_id', $business_id)->where('type', 'expense')->where('expense_category_id', $item->id)->sum('final_total'), 
            ];
        }

        $business_locations = BusinessLocation::forDropdown($business_id, true);

        return view('report.trial_balance', compact('business_locations', 'data', 'start_date', 'end_date', 'location_id'));
    }
    ...
    public function getRegisterReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($registers)
                ...
                ->editColumn('safe_staus', function ($row) {
                    return __($row->safe_status);
                })
                ->editColumn('for_user_id', function ($row) {
                    return implode(" ", [optional($row->for_user)->first_name, optional($row->for_user)->last_name]);
                })
                ...
                ->addColumn('action', function($row){
                    return view('report.partials.register_cash_action', compact("row"));
                })
            ...
            return Datatables::of($query)
                ...
                ->editColumn('ref_no', '
                    @if($purchase_type == "opening_stock")
                        <i><small class="help-block">(@trans("lang_v1.opening_stock"))</small></i>
                    @else
                        {{$ref_no}}
                    @endif
                    ')
    ...
    public function purchasePaymentReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($query)
                ...
                ->addColumn('action', '<button type="button" class="btn btn-primary btn-xs view_payment" data-href="{{ action("TransactionPaymentController@viewPayment", [$DT_RowId]) }}">@trans("messages.view")
                    </button> @if(!empty($document))<a href="{{asset("/uploads/documents/" . $document)}}" class="btn btn-success btn-xs" download=""><i class="fa fa-download"></i> @trans("purchase.download_document")</a>@endif')
