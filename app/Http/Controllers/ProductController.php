...
class ProductController extends Controller
{
    ...
    public function index()
    {
        ...
        if (request()->ajax()) {
            ...
            $products->whereNull('products.class_type_id');
            ...
            return Datatables::of($products)
                ...
                ->addColumn(
                    ...
                    function ($row) use ($selling_price_group_count) {
                        ...
                        if (auth()->user()->can('product.update')) {
                            $html .=
                            '<li><a href="javascript:editProduct('.$row->id.')" _href="' . action('ProductController@edit', [$row->id]) . '"><i class="glyphicon glyphicon-edit"></i> ' . __("messages.edit") . '</a></li>';
                        }
                        ...
                        if (auth()->user()->can('product.create')) {
                            ...
                            $html .=
                                '<li><a href="javascript:createProduct(\'d='.$row->id.'\')" _href="' . action('ProductController@create', ["d" => $row->id]) . '"><i class="fa fa-copy"></i> ' . __("lang_v1.duplicate_product") . '</a></li>';
                        }
                ...
                ->editColumn('image', function ($row) {
                    return '<div style="display: flex;"  ><img src="' . $row->image_url . '" alt="Product image" style="border-radius: 5em;width: 40px;height: 40px;" class="sb-shadow"></div>';
                })
                ->editColumn('account_id', function ($row) {
                    return optional(Account::find($row->account_id))->name;
                })
                ...
                ->addColumn('mass_delete', function ($row) {
                    return  '<input type="checkbox" style="width: 50px" class="row-select no-icheck" value="' . $row->id .'">' ;
                })
    ...
    public function create()
    {
        ...
        return view('product.form')
            ->with(compact('product', 'categories', 'brands', 'units', 'taxes', 'barcode_types', 'default_profit_percent', 'tax_attributes', 'barcode_default', 'business_locations', 'duplicate_product', 'sub_categories', 'rack_details', 'selling_price_group_count', 'module_form_parts', 'product_types', 'common_settings', 'warranties', 'pos_module_data'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            $business_id = $request->session()->get('user.business_id');
            $form_fields = ['account_id', 'class_type_id', 'name', 'brand_id', 'unit_id', 'category_id', 'tax', 'type', 'barcode_type', 'sku', 'alert_quantity', 'tax_type', 'weight', 'product_custom_field1', 'product_custom_field2', 'product_custom_field3', 'product_custom_field4', 'product_description', 'sub_unit_ids'];
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            $business_id = $request->session()->get('user.business_id');
            $product_details = $request->only(['account_id', 'name', 'brand_id', 'unit_id', 'category_id', 'tax', 'barcode_type', 'sku', 'alert_quantity', 'tax_type', 'weight', 'product_custom_field1', 'product_custom_field2', 'product_custom_field3', 'product_custom_field4', 'product_description', 'sub_unit_ids']);
            ...
            $product->account_id = isset($product_details['account_id'])? $product_details['account_id'] : null;
            $product->unit_id = isset($product_details['unit_id'])? $product_details['unit_id'] : null;
