...
<body> 
  <a href="{{ url('/') }}" style="margin: 30px">
    ...
    <div class="container">
        ...
        <div class="feedback">
          <!--
          <img src="{{ url('/images/sub/rate.png') }}" style="width: 100px;margin: auto" alt="">
          -->
          <h3>{{ $rater->name }}</h3>
          <h6>{{ $rater->description }}</h6>

          <div class="rating">
            <input type="hidden" name="" id="rateValue" >
          ...
          <br>
          <input type="text" class="form-control" style="border-radius: 7px" value="{{ optional($rate)->comment }}" id="comment" placeholder="@trans('Tell Me Why If You Want ?')" >
          <br>
      ...
      <script>
        ...
        $('.rate-btn').click(function(){
          ...
          if (rate <= 0)
            return toastr.error('@trans("please rate") {{ $rater->name }}');
          ...
          $.post("{{ url('/set-rate') }}/{{ $rater->id }}?", $.param(data), function(r){
            if (r.status == 1) {
