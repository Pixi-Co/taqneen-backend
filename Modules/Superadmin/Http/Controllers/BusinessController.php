...
class BusinessController extends BaseController
{
    ...
    public function index()
    {
        ...
        if (request()->ajax()) {
            ...
            return Datatables::of($query)
                ...
                ->editColumn( 'is_active', '@if($is_active == 1) <span class="label bg-green">@trans("business.is_active")</span> @else <span class="label bg-gray">@trans("lang_v1.inactive")</span> @endif')
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            //Create owner.
           $owner_details = $request->only(['businesstype_id', 'surname', 'first_name', 'last_name', 'username', 'email', 'password']);
        ...
        if (request()->ajax()) {
            ...
            return Datatables::of($users)
                ...
                ->addColumn(
                    'action',
                    '@can("user.update")
                        <a href="#" class="btn btn-xs btn-primary update_user_password" data-user_id="{{$id}}" data-user_name="{{$full_name}}"><i class="glyphicon glyphicon-edit"></i> @trans("superadmin::lang.update_password")</a>
                        &nbsp;
                    @endcan'
                )
