<div class="modal-dialog modal-xl" role="document">
	<div class="modal-content">
		<div class="modal-header">
		    ...
		    <h4 class="modal-title" id="modalTitle"> @trans('lang_v1.stock_transfer_details') (<b>@trans('purchase.ref_no'):</b> #{{ $sell_transfer->ref_no }})
		    </h4>
		</div>
		<div class="modal-body">
				<div class="row invoice-info">
				  <div class="col-sm-4 invoice-col">
				    @trans('lang_v1.location_from'):
				    <address>
				      ...
				      @if(!empty($sell_transfer->contact->tax_number))
				        <br>@trans('contact.tax_no'): {{$sell_transfer->contact->tax_number}}
				      @endif

				      @if(!empty($location_details['sell']->mobile))
				        <br>@trans('contact.mobile'): {{$location_details['sell']->mobile}}
				      @endif
				  ...
				  <div class="col-md-4 invoice-col">
				    ...
				    <address>
				      ...
				      @if(!empty($location_details['purchase']->mobile))
				        <br>@trans('contact.mobile'): {{$location_details['purchase']->mobile}}
				      @endif
				  ...
				  <div class="col-sm-4 invoice-col">
				    <b>@trans('purchase.ref_no'):</b> #{{ $sell_transfer->ref_no }}<br/>
				    <b>@trans('messages.date'):</b> {{ @format_date($sell_transfer->transaction_date) }}<br/>
				    <b>@trans('sale.status'):</b> {{$statuses[$sell_transfer->status] ?? ''}}
				  </div>
				</div>
				            ...
				            <td>
				              {{ $sell_lines->product->name }}
				               ...
				               @if($lot_n_exp_enabled && !empty($sell_lines->lot_details))
				                <br>
				                <strong>@trans('lang_v1.lot_n_expiry'):</strong> 
