<div class="modal-header">
    ...
    <h4 class="modal-title" id="modalTitle"> @trans('purchase.purchase_details') (<b>@trans('purchase.ref_no'):</b> #{{ $purchase->ref_no }})
    </h4>
</div>
<div class="modal-body">
  <div class="row">
    <div class="col-sm-12">
      <p class="pull-right"><b>@trans('messages.date'):</b> {{ @format_date($purchase->transaction_date) }}</p>
    </div>
  </div>
    ...
    <div class="col-sm-4 invoice-col">
      ...
      <address>
        ...
        @if(!empty($purchase->contact->tax_number))
          <br>@trans('contact.tax_no'): {{$purchase->contact->tax_number}}
        @endif
        @if(!empty($purchase->contact->mobile))
          <br>@trans('contact.mobile'): {{$purchase->contact->mobile}}
        @endif
        @if(!empty($purchase->contact->email))
          <br>@trans('business.email'): {{$purchase->contact->email}}
        @endif
      </address>
    ...
    <div class="col-sm-4 invoice-col">
      ...
      <address>
        ...
        @if(!empty($purchase->location->mobile))
          <br>@trans('contact.mobile'): {{$purchase->location->mobile}}
        @endif
        @if(!empty($purchase->location->email))
          <br>@trans('business.email'): {{$purchase->location->email}}
        @endif
      </address>
    </div>

    <div class="col-sm-4 invoice-col">
      <b>@trans('purchase.ref_no'):</b> #{{ $purchase->ref_no }}<br/>
      <b>@trans('messages.date'):</b> {{ @format_date($purchase->transaction_date) }}<br/>
      <b>@trans('purchase.purchase_status'):</b> {{ __('lang_v1.' . $purchase->status) }}<br>
      <b>@trans('purchase.payment_status'):</b> {{ __('lang_v1.' . $purchase->payment_status) }}<br>
    </div>
  </div>
        ...
        <table class="table bg-gray">
          <thead>
            <tr class="bg-green">
              ...
              <th class="text-right">@trans('purchase.purchase_quantity')</th>
              <th class="text-right">@trans( 'lang_v1.unit_cost_before_discount' )</th>
              <th class="text-right">@trans( 'lang_v1.discount_percent' )</th>
              <th class="no-print text-right">@trans('purchase.unit_cost_before_tax')</th>
              <th class="no-print text-right">@trans('purchase.subtotal_before_tax')</th>
              <th class="text-right">@trans('sale.tax')</th>
              <th class="text-right">@trans('purchase.unit_cost_after_tax')</th>
              <th class="text-right">@trans('purchase.unit_selling_price')</th>
              ...
              @endif
              <th class="text-right">@trans('sale.subtotal')</th>
            </tr>
          </thead>
          @php 
  ...
  <div class="row">
    ...
    <div class="col-md-6 col-sm-12 col-xs-12">
      <div class="table-responsive">
        <table class="table">
          <!-- <tr class="hide">
            <th>@trans('purchase.total_before_tax'): </th>
            <td></td>
            <td><span class="display_currency pull-right">{{ $total_before_tax }}</span></td>
          </tr> -->
          <tr>
            <th>@trans('purchase.net_total_amount'): </th>
            <td></td>
          ...
          <tr>
            <th>@trans('purchase.purchase_tax'):</th>
            <td><b>(+)</b></td>
            <td class="text-right">
          ...
          @if( !empty( $purchase->shipping_charges ) )
            <tr>
              <th>@trans('purchase.additional_shipping_charges'):</th>
              <td><b>(+)</b></td>
              <td><span class="display_currency pull-right" >{{ $purchase->shipping_charges }}</span></td>
            </tr>
          @endif
          <tr>
            <th>@trans('purchase.purchase_total'):</th>
            <td></td>
  ...
  <div class="row">
    <div class="col-sm-6">
      <strong>@trans('purchase.shipping_details'):</strong><br>
    ...
    <div class="col-sm-6">
      <strong>@trans('purchase.additional_notes'):</strong><br>
      <p class="well well-sm no-shadow bg-gray">
