...
@php
  $p = App\Product::find($product_deatails->product_id);
@endphp
...
<div class="col-sm-12"><br>
    ...
    <table class="table table-bordered add-product-price-table table-condensed {{$class}}">
        <tr>
          <th class="{{ $p->class_type_id > 0? 'hidden' : '' }}" >@trans('product.default_purchase_price')</th>
          <th class="{{ $p->class_type_id > 0? 'hidden' : '' }}" >@trans('product.profit_percent') @show_tooltip(__('tooltip.profit_percent'))</th>
        ...
        @foreach($product_deatails->variations as $variation )
            @if($loop->first)
                <tr>
                    <td class="{{ $p->class_type_id > 0? 'hidden' : '' }}" >
                    ...
                    <td>
                        ...
                        <div class="form-group">
                            ...
                            <small><p class="help-block">@trans('purchase.max_file_size', ['size' => (config('constants.document_size_limit') / 1000000)]) <br> @trans('lang_v1.aspect_ratio_should_be_1_1')</p></small>
                        </div>
                    </td>
                </tr>
            @endif
