					...
					<tr>
						<td>
							<div class="{{$col}} col-xs-6 d-inline-table">
								...
								(-):</b> 
								<br/>
								<i class="fas fa-pen cursor-pointer" id="pos-edit-discount" title="@trans('sale.edit_discount')" aria-hidden="true" data-toggle="modal" data-target="#posEditDiscountModal"></i>
							...
							<div class="{{$col}} col-xs-6 d-inline-table">
								...
								<b>@trans('sale.order_tax')(+): @show_tooltip(__('tooltip.sale_tax'))</b>
								<br/>
								<i class="fas fa-pen cursor-pointer" title="@trans('sale.edit_order_tax')" aria-hidden="true" data-toggle="modal" data-target="#posEditOrderTaxModal" id="pos-edit-tax" ></i> 
								<span id="order_tax">
									@if(empty($edit))
										0
									@else
							...
							<div class="{{$col}} col-xs-6 d-inline-table">
								...
								<b>@trans('sale.shipping')(+): @show_tooltip(__('tooltip.shipping'))</b> 
								<br/>
								<i class="fas fa-pen cursor-pointer"  title="@trans('sale.shipping')" aria-hidden="true" data-toggle="modal" data-target="#posShippingModal"></i>
							...
							@if(in_array('types_of_service', $enabled_modules))
								<div class="col-sm-3 col-xs-6 d-inline-table">
									<b>@trans('lang_v1.packing_charge')(+):</b>
									<br/>
							...
							<div class="col-sm-3 col-xs-12 d-inline-table">
								<b>@trans('sale.total_payable'):</b>
								<small class="hide" id="round_off"><br>(@trans('lang_v1.round_off'): <span id="round_off_text"></span>)</small>
								<br/>
								...
								@if(empty($edit))
									<button type="button" class="btn btn-danger btn-flat btn-xs pull-right" id="pos-cancel">@trans('sale.cancel')</button>
								@else
									<button type="button" class="btn btn-danger btn-flat hide btn-xs pull-right" id="pos-delete">@trans('messages.delete')</button>
								@endif
							</div>
						</td>
					</tr>
						...
						<td>
							<div class="col-sm-2 col-xs-6 col-2px-padding">
								...
								<button type="button" 
									...
									id="pos-draft">@trans('sale.draft')</button>

								<button type="button" 
									class="btn btn-info btn-block btn-flat" 
									id="pos-quotation">@trans('lang_v1.quotation')</button>
							</div>
							<div class="col-sm-3 col-xs-6 col-2px-padding">
								@if(!empty($pos_settings['show_credit_sale_button']))
									...
									title="@trans('lang_v1.tooltip_credit_sale')" >
									<div class="text-center">
										<i class="fa fa-check" aria-hidden="true"></i>
	    								<b>@trans('lang_v1.credit_sale')</b>
	    							</div>
									</button>
								@else
									<button type="button" 
									...
									title="@trans('lang_v1.tooltip_express_checkout_card')" >
									<div class="text-center">
										<i class="fa fa-check" aria-hidden="true"></i>
	    								<b>@trans('lang_v1.express_checkout_card')</b>
	    							</div>
									</button>
								@endif
								@if(empty($pos_settings['disable_suspend']))
									...
									title="@trans('lang_v1.tooltip_suspend')" >
									<div class="text-center">
										<i class="fa fa-pause" aria-hidden="true"></i>
	    								<b>@trans('lang_v1.suspend')</b>
	    							</div>
									</button>
								@endif
							</div>
							<div class="col-sm-4 col-xs-12 col-2px-padding">
								<button type="button" class="btn bg-navy  btn-block btn-flat btn-lg no-print @if($pos_settings['disable_pay_checkout'] != 0) hide @endif pos-express-btn" id="pos-finalize" title="@trans('lang_v1.tooltip_checkout_multi_pay')">
								<div class="text-center">
									<i class="fa fa-check" aria-hidden="true"></i>
    								<b>@trans('lang_v1.checkout_multi_pay')</b>
    							</div>
								</button>
							</div>
							<div class="col-sm-3 col-xs-12 col-2px-padding">
								...
								title="@trans('tooltip.express_checkout')">
								<div class="text-center">
									<i class="fa fa-check" aria-hidden="true"></i>
    								<b>@trans('lang_v1.express_checkout_cash')</b>
    							</div>
								</button>
							</div>
...
@if(isset($transaction))
	@include('sub_pos.partials.edit_discount_modal', ['sales_discount' => $transaction->discount_amount, 'discount_type' => $transaction->discount_type, 'rp_redeemed' => $transaction->rp_redeemed, 'rp_redeemed_amount' => $transaction->rp_redeemed_amount, 'max_available' => !empty($redeem_details['points']) ? $redeem_details['points'] : 0])
@else
	@include('sub_pos.partials.edit_discount_modal', ['sales_discount' => $business_details->default_sales_discount, 'discount_type' => 'percentage', 'rp_redeemed' => 0, 'rp_redeemed_amount' => 0, 'max_available' => 0])
@endif

@if(isset($transaction))
	@include('sub_pos.partials.edit_order_tax_modal', ['selected_tax' => $transaction->tax_id])
@else
	@include('sub_pos.partials.edit_order_tax_modal', ['selected_tax' => $business_details->default_sales_tax])
@endif

@include('sub_pos.partials.edit_shipping_modal')
