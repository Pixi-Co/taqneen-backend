<div class="w3-">
	<div class="row">
		...
		<div class="col-md-6 w3-padding hidden">
				...
				<div class="input-group"> 
					...
					<span class="input-group-btn">
						...
						@if(isset($pos_settings['enable_weighing_scale']) && $pos_settings['enable_weighing_scale'] == 1)
							...
							title="@trans('lang_v1.weighing_scale')"><i class="fa fa-digital-tachograph fa-lg"></i></button>
						@endif
	...
	<div class="row">
		...
		<div class="col-md-4 w3-padding hidden">
			<div class="form-group">
			{!! Form::select('invoice_layout_id', 
						$invoice_layouts, optional(App\InvoiceLayout::getLayoutOfSubscription())->id, ['class' => 'form-control select2', 'placeholder' => __('lang_v1.select_invoice_layout'), 'id' => 'invoice_layout_id']); !!}
			</div>
		</div>
		@endif   
		...
		<div class="col-md-4 w3-padding"    >
			<div class="form-group">
			<b>@trans('subscription status')</b>
			{!! Form::select('subscription_status', Subscription::$subscription_status, null, ['class' => 'form-control', 'id' => 'subscription_status']); !!}
			</div>
		</div> 
		...
		<div class="col-md-4 w3-padding"    >
			<b>@trans('source')</b>
			<div class="form-group">
			{!! Form::select('source', Subscription::getSources(), null, ['class' => 'form-control', 'id' => 'subscription_status']); !!}
			</div>
		</div> 
  
		<div class="col-md-6 w3-padding" id="class_type_div"   >
			<div class="form-group">
				<b>@trans('service')</b>
				{!! Form::select('class_type_id', Modules\Subscription\Entities\ClassType::active()->pluck('name', 'id')->toArray(), null, ['class' => 'form-control', 'placeholder' => __('select class type'), 'id' => 'class_type_id', 'onchange' => "$('.membership-option').hide();$('.membership-' + this.value).show()"]); !!}
			</div>
		</div> 
		...
		<div class="col-md-6 w3-padding"    >
			<div class="form-group">
				...
				<select class="form-control" name="product_id" onchange="chooseProduct('#productModal' + this.value);$('#class_type_div select').val('')" >
					<option value="">@trans('select membership')</option>
					@foreach(Subscription::memberShips()->get() as $item)
						<option class="membership-option membership-{{ $item->class_type_id }}" value="{{ $item->id }}">{{ $item->name }}</option>
					@endforeach
				</select> 
			</div>
		</div>
		...
		@if(!empty($commission_agent))
			<div class="col-md-4 w3-padding  commission_agent_div">
				<div class="form-group">
		...
		@if(!empty($price_groups) && count($price_groups) > 1)
			<div class="col-md-4 col-sm-6 w3-padding hidden">
				<div class="form-group">
					<div class="input-">
						<span class="input-group-addon hidden">
		...
		@if(in_array('types_of_service', $enabled_modules) && !empty($types_of_service))
			<div class="col-md-4 col-sm-6 w3-padding hidden">
				<div class="form-group">
					...
					<small><p class="help-block hide" id="price_group_text">@trans('lang_v1.price_group'): <span></span></p></small>
				</div>
			</div>
		...
		@if(in_array('subscription', $enabled_modules))
			<div class="col-md-4 col-sm-6 w3-padding hidden">
				<label>
				  {!! Form::checkbox('is_recurring', 1, false, ['class' => 'input-icheck', 'id' => 'is_recurring']); !!} @trans('lang_v1.subscribe')?
				</label>
		...
		@if(in_array('tables' ,$enabled_modules) || in_array('service_staff' ,$enabled_modules))
			...
			<span class="col-md-8 col-sm-12 w3-padding hidden" id="restaurant_module_span">
