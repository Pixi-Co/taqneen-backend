		...
		<div class="modal-body">
			<div class="row">
				<div class="form-group col-xs-12 @if(!auth()->user()->can('edit_product_price_from_sale_screen')) hide @endif">
					<label>@trans('sale.unit_price')</label>
				...
				@if(!auth()->user()->can('edit_product_price_from_sale_screen'))
					<div class="form-group col-xs-12">
						<strong>@trans('sale.unit_price'):</strong> {{@num_format(!empty($product->unit_price_before_discount) ? $product->unit_price_before_discount : $product->default_sell_price)}}
					</div>
				@endif
				<div class="form-group col-xs-12 col-sm-6 @if(!$edit_discount) hide @endif">
					<label>@trans('sale.discount_type')</label>
				...
				<div class="form-group col-xs-12 col-sm-6 @if(!$edit_discount) hide @endif">
					<label>@trans('sale.discount_amount')</label>
				...
				@if(!empty($warranties))
					<div class="form-group col-xs-12">
						<label>@trans('lang_v1.warranty')</label>
				...
				@endif
				<div class="form-group col-xs-12">
		      		<label>@trans('lang_v1.description')</label>
		      		<textarea class="form-control" name="products[{{$row_count}}][sell_line_note]" rows="3">{{$sell_line_note}}</textarea>
		      		<p class="help-block">@trans('lang_v1.sell_line_description_help')</p>
		      	</div>
			</div>
		</div>
