				...
				<div class="box-header with-border">
					<h3 class="box-title">
						...
						@if($transaction->status == 'draft' && $transaction->is_quotation == 1) 
							@trans('lang_v1.quotation')
						@elseif($transaction->status == 'draft') 
							Draft 
						@elseif($transaction->status == 'final') 
							Invoice 
						@endif 
					<span class="text-success">#{{$transaction->invoice_no}}</span> <i class="fa fa-keyboard-o hover-q text-muted" aria-hidden="true" data-container="body" data-toggle="popover" data-placement="bottom" data-content="@include('sub_pos.partials.keyboard_shortcuts_details')" data-html="true" data-trigger="hover" data-original-title="" title=""></i></h3>
					<p><strong>@trans('sale.location'):</strong> {{$transaction->location->name}}</p>
					<div class="pull-right box-tools">
                <a class="btn btn-success btn-sm" href="{{action('SubSellPosController@create')}}">
                  <strong><i class="fa fa-plus"></i> POS</strong></a>
              </div>
				</div>
				<input type="hidden" id="item_addition_method" value="{{$business_details->item_addition_method}}">
				{!! Form::open(['url' => action('SubSellPosController@update', [$transaction->id]), 'method' => 'post', 'id' => 'edit_pos_sell_form' ]) !!}
...
@endforeach
					...
					@include('sub_pos.partials.pos_details', ['edit' => true])

					@include('sub_pos.partials.payment_modal')

					@if(empty($pos_settings['disable_suspend']))
						@include('sub_pos.partials.suspend_note_modal')
					@endif

					@if(empty($pos_settings['disable_recurring_invoice']))
						@include('sub_pos.partials.recurring_invoice_modal')
					@endif
				</div>
				<!-- /.box-body -->
				{!! Form::close() !!}
		...
		<div class="col-md-5 col-sm-12">
			@include('sub_pos.partials.right_div')
		</div>
	</div>
</section>
