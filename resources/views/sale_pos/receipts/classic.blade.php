...
@section('content')
    ...
    <div class="w3-">
            ...
            <tr>
                <td class="text-left w3-padding">
                    ...
                    @if ($receipt_details->show_barcode)
                        <div class="qrcode" data-width="100" data-height="100"
                            data-text="{{ $receipt_details->url }}"></div>
                        <img class="" style="width: 160px;display: none"
                    ...
                    @if (!empty($receipt_details->invoice_heading))
                        <b
                            style="width: 125px;text-transform: uppercase;border-bottom: 2px dashed darkgray;color: black;font-size: 30px;letter-spacing: 5px">
                    ...
                    @endif
                    <br>
                    <span style="width: auto;text-transform: uppercase;font-size: 20px;">
        ...
        <table class="w3-table">
            <tr>
                <td class="text-left w3-padding" style="width: 35%">
                    ...
                    <!-- commercial_number -->
                    @if (!empty($receipt_details->commercial_number))
                        <h6 style="text-transform: capitalize">
        ...
        <table class="w3-table w3-bordered">
            <thead>
                <tr>
                    <th class="text-left" width="40%">{{ $receipt_details->table_product_label }}</th>
                    <th class="text-left" width="20%">{{ $receipt_details->table_qty_label }}</th>
                    <th class="text-left" width="20%">{{ $receipt_details->table_unit_price_label }}</th>
                    <th class="text-left" width="20%">{{ $receipt_details->table_subtotal_label }}</th>
                </tr>
            </thead>
            <tbody>
                @forelse($receipt_details->lines as $line)
                    <tr>
                        ...
                        <td class="text-left" >{{ $line['quantity'] }} {{ $line['units'] }} </td>
                        <td class="text-left" >{{ $line['unit_price_inc_tax'] }}</td>
                        <td class="text-left" >{{ $line['line_total'] }}</td>
                    </tr>
                    @if (!empty($line['modifiers']))
                        @foreach ($line['modifiers'] as $modifier)
                            <tr>
                                ...
                                <td class="text-left" >{{ $modifier['quantity'] }} {{ $modifier['units'] }}
                                </td>
                                <td class="text-left" >{{ $modifier['unit_price_inc_tax'] }}</td>
                                <td class="text-left" >{{ $modifier['line_total'] }}</td>
                            </tr>
                        @endforeach
                    @endif
                @empty
        ...
        <div class="w3-row">
            <div class="w3-col l3 m3 s3" style="padding: 5px" > 

                @if (!empty($receipt_details->qrcode_img))
                <div class="w3-center" style="padding: 5px">
                        <img style="width: 100%;margin-right: 0px" src="{{ $receipt_details->qrcode_img }}"
                            class="img img-responsive center-block">
                            <hr>
            ...
            @if (!empty($receipt_details->footer_text))
                <div class="w3-col l9 m9 s9 " style="padding: 5px">

                    <div class="row w3-tiny" style="padding: 5px">
                        <div class="col-xs-12 w3-tiny">
