...
@if(!empty($status) && $status != $old_status)
    <tr>
        <th class="width-50">@trans('sale.status'): </th> 
...
@if(!empty($shipping_status) && $shipping_status != $old_shipping_status)
    <tr>
        <th class="width-50">@trans('lang_v1.shipping_status'): </th> 
        <td class="width-50 text-left">
...
@if(!empty($final_total) && $final_total != $old_final_total)
    <tr>
    <th class="width-50">@trans('sale.total'): </th> 
    <td class="width-50 text-left">
...
@if(!empty($payment_status) && $payment_status != $old_payment_status)
    <tr>
        <th class="width-50">@trans('sale.payment_status'): </th> 
        <td class="width-50 text-left">
            @if(!empty($old_payment_status))
                <span class="label bg-info">@trans('lang_v1.' . $old_payment_status)</span> --> 
            @endif
                <span class="label bg-info">@trans('lang_v1.' . $payment_status)</span>
        </td>
    </tr>
@endif
