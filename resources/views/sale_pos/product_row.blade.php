<tr class="product_row product-row-number-{{ $row_count }}" data-row_index="{{ $row_count }}">
    <td>
        ...
        @if (($edit_price || $edit_discount) && empty($is_direct_sell))
            <div title="@trans('lang_v1.pos_edit_product_price_help')" class=" w3-text-dark-gray">
                <a class="text-link text-info cursor-pointer w3-text-dark-gray" data-toggle="modal"
                    data-target="#row_edit_product_price_modal_{{ $row_count }}">
                    <img src="
     @if (count($p->media) > 0) {{ $p->media->first()->display_url }}
					@elseif(!empty($p->product_image))
						{{ asset('/uploads/img/' . rawurlencode($p->product_image)) }}
					@else
						@php
							$image = optional(App\Product::find($product->product_id))->image_url; @endphp
      {{ $image ? $image : asset('/img/product-default-image.jpg') }}
     @endif" style="width: 50px;height: 50px;border-radius: 5em">

                    <b style="padding: 5px">{!! $productItem->name !!}
                        <span class="text-link text-info cursor-pointer w3-text-dark-gray hidden" data-toggle="modal"
                            data-target="#row_edit_product_price_modal_{{ $row_count }}">
     ...
     -->
        ...
        @if (session()->get('business.enable_lot_number') == 1 || session()->get('business.enable_product_expiry') == 1)
            ...
            @if (!empty($product->lot_numbers))
                <select class="hidden form-control lot_number input-sm"
                    ...
                    @foreach ($product->lot_numbers as $lot_number)
                        ...
                        <option value="{{ $lot_number->purchase_line_id }}"
                            ...
                            data-msg-max="@trans('lang_v1.quantity_error_msg_in_lot', ['qty'=> $lot_number->qty_formated, 'unit' => $product->unit  ])"
                            {{ $selected }}>
                            @if (!empty($lot_number->lot_number) && $lot_enabled == 1)
                                ...
                                @endif @if ($lot_enabled == 1 && $exp_enabled == 1)
                                    ...
                                    @endif @if ($exp_enabled == 1 && !empty($lot_number->exp_date))
                                        @trans('product.exp_date'): {{ @format_date($lot_number->exp_date) }}
                                    @endif {{ $expiry_text }}
                        </option>
                    @endforeach
                </select>
    ...
    @if (count($sub_units) > 1)
        <td>
            <select name="products[{{ $row_count }}][sub_unit_id]" style="width: 70px" class="w3-input sub_unit">
    ...
    <td>
        ...
        <input type="hidden" name="products[{{ $row_count }}][unit_price]" class="hidden_product_unit_price"
            value="{{ @num_format(!empty($product->unit_price_before_discount) ? $product->unit_price_before_discount : $product->default_sell_price) }}">
        <input type="hidden" name="products[{{ $row_count }}][item_tax]" class="hidden_product_unit_price"
            value="{{ $product->tax }}">
        <input type="hidden" name="products[{{ $row_count }}][tax_type]" class="hidden_product_unit_price"
            value="{{ $product->tax_type }}">
        <input type="hidden" name="products[{{ $row_count }}][tax]" class="hidden_product_tax_id"
            value="{{ $product->tax }}">
        ...
        <div class="input-group input-number">
            <span class="input-group-btn"><button type="button" class="btn btn-default btn-flat quantity-down"><i
                        class="fa fa-minus text-"></i></button></span>
			...
			@else 
				...
				data-msg-abs_digit="@trans('lang_v1.decimal_value_not_allowed')" @endif
                data-rule-required="true"
                data-msg-required="@trans('validation.custom-messages.this_field_is_required')"
                @if ($product->enable_stock && empty($pos_settings['allow_overselling'])) data-rule-max-value="{{ $max_qty_rule }}" data-qty_available="{{ $product->qty_available }}" data-msg-max-value="{{ $max_qty_msg }}" 
				data-msg_max_default="@trans('validation.custom-messages.quantity_not_available', ['qty'=> $product->formatted_qty_available, 'unit' => $product->unit  ])" @endif>
            <span class="input-group-btn"><button type="button" class="btn btn-default btn-flat quantity-up"><i
                        class="fa fa-plus text-"></i></button></span>
        </div>
        ...
        @if ($productItem->variations->count() > 1)
            <div class="pos_variations" style="padding: 5px">
                <a href="#"
                    ...
                    class="pt-3 w3-" id="pos-edit-order_details" title="Edit " aria-hidden="true"
                    data-toggle="modal" data-target="#posEditOrderDetails">
    ...
    <td style="padding: 12px">
        <p class="font-weight-bolder">
            <b><b data-price="{{ $product->default_sell_price }}"
                    class="row-total">{{ $product->default_sell_price * (isset($quantity) ? $quantity : $product->quantity) }}</b></b>
        </p>
    </td>
    <td class="text-center " style="padding: 12px">
        <i class="fa fa-times text- pos_remove_row cursor-pointer btn btn-default" aria-hidden="true"></i>
    </td>
</tr>
