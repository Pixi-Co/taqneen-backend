              ...
              {!! Form::open(['url' => action('ReportController@getTrendingProducts'), 'method' => 'get' ]) !!}
               <div class="w3-padding">
                ...
                @endcanaccessmodule
                    ...
                    <div class="col-sm-12">
                      <button type="submit" class="btn btn-primary pull-right">@trans('report.apply_filters')</button>
                    </div> 
                   </div>
               </div>
                {!! Form::close() !!}
    ...
    <div class="row">
        <div class="col-xs-12">
            @component('components.widget', ['class' => 'box-primary'])
                @slot('title')
                    @trans('report.top_trending_products') @show_tooltip(__('tooltip.top_trending_products'))
                @endslot
                {!! $chart->container() !!}
            @endcomponent
        </div>
