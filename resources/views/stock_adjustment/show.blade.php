<div class="modal-dialog modal-xl" role="document">
	<div class="modal-content">
		<div class="modal-header">
		    ...
		    <h4 class="modal-title" id="modalTitle"> @trans('lang_v1.stock_adjustment_details') (<b>@trans('purchase.ref_no'):</b> #{{ $stock_adjustment->ref_no }})
		    </h4>
		</div>
		<div class="modal-body">
		  	<div class="row">
			    <div class="col-sm-12">
			      <p class="pull-right"><b>@trans('messages.date'):</b> {{ @format_date($stock_adjustment->transaction_date) }}</p>
			    </div>
			</div>
			<div class="row invoice-info">
				<div class="col-sm-4 invoice-col">
			        ...
			        @if(!empty($stock_adjustment->location->mobile))
			          <br>@trans('contact.mobile'): {{$stock_adjustment->location->mobile}}
			        @endif
			        @if(!empty($stock_adjustment->location->email))
			          <br>@trans('business.email'): {{$stock_adjustment->location->email}}
			        @endif
			      </address>
			    </div>

			    <div class="col-sm-4 invoice-col">
			      	<b>@trans('purchase.ref_no'):</b> #{{ $stock_adjustment->ref_no }}<br/>
			      	<b>@trans('messages.date'):</b> {{ @format_date($stock_adjustment->transaction_date) }}<br/>
			      	<b>@trans('stock_adjustment.adjustment_type'):</b> {{ __('stock_adjustment.' . $stock_adjustment->adjustment_type) }}<br>
			      	<b>@trans('stock_adjustment.reason_for_stock_adjustment'):</b> {{ $stock_adjustment->additional_notes }}<br>
			    </div>
    		</div>
