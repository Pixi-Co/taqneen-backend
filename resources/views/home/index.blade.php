...
@section('content')
    ...
    <section class="content content-custom no-print">  
        ...
        @if (can_bt(['subscription.dashboard']) )
            @if (auth()->user()->can('dashboard.data'))
                @include("subscription::dashboard.content", ['data' => $data])
            @else
                @include("home.cashier")
            @endif 
        @endif
...
@section('javascript')
    ...
    @if (auth()->user()->can('dashboard.data'))
        <script>
            ...
            var topSellerMemberShipData = {!! json_encode($data['topseller_membership']['data']) !!}; 
            var topSellerMemberShipLabels = {!! json_encode($data['topseller_membership']['labels']) !!};
        </script>
    ...
    -->
        ...
        <script>
            $(document).ready(function() {
                ...
                __currency_convert_recursively($('.home'));
        ...
        @can_bt(['subscription.dashboard'])
        @php 
            $resource = $data['subscription']['chart_data'];  
            $resource2 = $data['subscription']['chart_data2'];  
          @endphp
        @include("subscription::user.profile_chart_js", ["id" => "subscriptions", "resource" => $resource, "title" => __('subscriptions')])
        @include("subscription::user.profile_chart_js", ["id" => "subscription_classtypes", "resource" => $resource2, "title" => __('subscriptions of class types')])
        @include("subscription::subscription.scripts", ["query" => "is_expire=1"])
        @endcan_bt
    @endif
    
    @php 
        $resourceCashier = $data['cashier']['chart_data'];   
    @endphp
    @include("subscription::user.profile_chart_js", ["id" => "cashier_sales", "resource" => $resourceCashier, "title" => __('sales')])
@endsection
